...
Static HTML + vanilla JS + Azure Maps SDK (CDN). No build step. Served from the same K8s pod as the backend (`romainecr.azurecr.io/house-hunt:<sha>`) via `express.static('frontend/')` with an SPA fallback — Dockerfile copies `frontend/` into the image after `generate-config.sh` writes `config.js` at build time. Full-screen dark-themed map (`night` style) with a collapsible right sidebar for property list and admin form. Pin markers via SymbolLayer with canvas-generated icons per status color (fixed pixel size at all zoom levels). Geocoding via Azure Maps Search API with `x-ms-client-id` header. RMLS link lookup auto-fills address, metadata, and listing photo.
